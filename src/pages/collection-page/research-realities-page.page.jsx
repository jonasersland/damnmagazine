...
const ResearchRealitiesPage = ({tags})=>{
    ...
    setTitleHandler('DAMN Magazine - Research & Realities')

    useEffect(()=>{
        // console.log(location.pathname)
        if(location.pathname.includes('/research-realities/')){
                let currentLocation = location.pathname.replace('/research-realities/','');
                // console.log(currentLocation) 
                if (!currentLocation){
                    setQuery(`{'articles': *[_type == 'researchreality']{_id,originallyPublished, _createdAt, _type, slug, title,thumbnail, doubleWidth,'highlightItem':highlightItem[]{'asset':asset->}[0],"video": [video.asset->{...}],'eventData':[{startingTime, endingTime, place}], 'author':*[_id == ^.author._ref]{title}} | order(_createdAt desc) | order(originallyPublished desc)}`)
                } else{
                    let optionCap = currentLocation.charAt(0).toUpperCase() + currentLocation.slice(1);
                    // console.log(optionCap);
                    setQuery(`{'articles': *[_type == 'researchreality' && "${optionCap}" in tags[].label] {_id,originallyPublished,_createdAt, _type, slug, title,thumbnail,doubleWidth,'highlightItem':highlightItem[]{'asset':asset->}[0],"video": [video.asset->{...}],'eventData':[{startingTime, endingTime, place}], 'author':*[_id == ^.author._ref]{title}} | order(_createdAt desc) | order(originallyPublished desc)}`)
                    
                    const formattedTag = (tag)=>{
                        if(tag == 'Institution' || tag == 'Institutions'){
                            return 'INSTITUTIONS'
                        }
                        if(tag == 'Company'){
                            return 'COMPANY NEWS'
                        }
                        if(tag == 'Product' || tag == 'Products'){
                            return 'PRODUCTS'
                        }
                    }
                    setActiveTag(formattedTag(optionCap))
                }
            }else{
                setQuery(`{'articles': *[_type == 'researchreality']{_id,originallyPublished, _createdAt, _type, slug, title,thumbnail, doubleWidth,'highlightItem':highlightItem[]{'asset':asset->}[0],"video": [video.asset->{...}],'eventData':[{startingTime, endingTime, place}], 'author':*[_id == ^.author._ref]{title}} | order(_createdAt desc) | order(originallyPublished desc)}`)
            }
    ...
    const pageNavigationHandler = (option)=>{
        ...
        if (!option){
            setActiveTag(null)
            navigate(`/research-realities/`)
        }else{
            let optionCap = option.charAt(0).toUpperCase() + option.slice(1);
            if(optionCap == 'INSTITUTION'){
                optionCap = 'Institution'
            }
            if(optionCap == 'COMPANY NEWS'){
                optionCap = 'Company'
            }
            if(optionCap == 'PRODUCT'){
                optionCap = 'Product'
            }
            navigate(`/research-realities/${optionCap}`)
        }

        // console.log(optionCap)
        // setQuery(`{'articles': *[_type == 'researchreality' && "${optionCap}" in tags[].label] {_id,originallyPublished,_createdAt, _type, slug, title,doubleWidth,'highlightItem':highlightItem[]{'asset':asset->}[0],"video": [video.asset->{...}],'eventData':[{startingTime, endingTime, place}], 'author':*[_id == ^.author._ref]{title}} | order(_createdAt desc) | order(originallyPublished desc)}`) 
        // setActiveTag(option)
    }
    ...
    //    console.log(pageContent.articles.length)
        ...
        if(returnColAmount() == 'fiveCol'){
            ...
            for (var i=0;pageContent.articles.length > i; i++){
                ...
                if(i == pageContent.articles.length - 1){
                    // console.log('at length, ', singleRow)
...
export default ResearchRealitiesPage