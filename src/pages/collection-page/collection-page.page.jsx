...
const CollectionPage = ({title})=>{
    ...
    useEffect(()=>{ 
        if(title == 'TAG'){
            ...
            setQuery(`{'articles': *[_type == 'article' && "${slugCapSpaced}" in tags[].label || _type == 'researchreality' && "${slugCapSpaced}" in tags[].label] {_id, originallyPublished, _createdAt, _type, slug, title,thumbnail,doubleWidth,'highlightItem':highlightItem[]{'asset':asset->}[0],"video": [video.asset->{...}],'eventData':[{startingTime, endingTime, place}], 'author':*[_id == ^.author._ref]{title}} | order(_createdAt desc) | order(originallyPublished desc)}`)    
    ...
    //     if (!option){
    //         setQuery(`{'articles': *[_type == 'article'] {_id, _type, slug, title,doubleWidth,'highlightItem':highlightItem[]{'asset':asset->}[0],"video": [video.asset->{...}],'eventData':[{startingTime, endingTime, place}], 'author':*[_id == ^.author._ref]{title}}}`)
    //         setActiveTag('')
    //     }
    //     let optionCap = option.charAt(0).toUpperCase() + option.slice(1);
    //     setQuery(`{'articles': *[_type == 'article' && "${optionCap}" in tags[].label] {_id, _type, slug, title,doubleWidth,'highlightItem':highlightItem[]{'asset':asset->}[0],"video": [video.asset->{...}],'eventData':[{startingTime, endingTime, place}], 'author':*[_id == ^.author._ref]{title}}}`)
    //     setActiveTag(option)
    // }
    ...
    const articleRows = ()=>{ // currently only shows full rows. In future, how to automatically fill gap if there's four elements
       console.log(pageContent.articles.length)
        if(pageContent.articles.length < 1) {console.log(pageContent.articles.length);return []}
