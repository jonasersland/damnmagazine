...
const CalendarPage = ()=>{
    ...
    const [query, setQuery] = useState(`{'articles': *[_type == 'event']{_id, originallyPublished, _createdAt, _type, slug, title,doubleWidth,'highlightItem':highlightItem[]{'asset':asset->}[0],"video": [video.asset->{...}],'eventData':[{startingTime, endingTime, place}], 'author':*[_id == ^.author._ref]{title}} | order(_createdAt desc) | order(originallyPublished desc)}`) 

    // const [query, setQuery] = useState(`{'articles': *[_type == 'event' && dateTime(endingTime) > dateTime(now()) ]{_id, _type, slug, title,doubleWidth,'highlightItem':highlightItem[]{'asset':asset->}[0],"video": [video.asset->{...}],'eventData':[{startingTime, endingTime, place}], 'author':*[_id == ^.author._ref]{title}} | order(startingTime asc)}`)
    const [activeTag, setActiveTag] = useState('')
    const [cities, setCities] = useState(['AMSTERDAM', 'BERLIN', 'BRUSSELS', 'LONDON', 'MILANO'])
    ...
    //     setActiveTag(tag);
    //     setSearchInput(tag);
    //     setQuery(`{'articles': *[_type == "event" && "${tag}" in place[].label]{_id, _type, slug, title,doubleWidth,'highlightItem':highlightItem[]{'asset':asset->}[0],"video": [video.asset->{...}],'eventData':[{startingTime, endingTime, place}], 'author':*[_id == ^.author._ref]{title}}}`)
    // },[location])
    ...
    const pageNavigationHandler = (option)=>{
        ...
        // console.log(option)
        if (!option){
            setQuery(`{'articles': *[_type == 'event'] {_id,originallyPublished,_createdAt, _type, slug, title,thumbnail,doubleWidth,'highlightItem':highlightItem[]{'asset':asset->}[0],"video": [video.asset->{...}],'eventData':[{startingTime, endingTime, city, place}], 'author':*[_id == ^.author._ref]{title}} | order(_createdAt desc) | order(originallyPublished desc)}`) 
            setActiveTag('')
        }
        let optionCap = option.charAt(0).toUpperCase() + option.slice(1).toLowerCase();
        // console.log(optionCap)
        setQuery(`{'articles': *[_type == "event" && "${optionCap}" in place[].label]{_id,originallyPublished,_createdAt, _type, slug, title,thumbnail,doubleWidth,'highlightItem':highlightItem[]{'asset':asset->}[0],"video": [video.asset->{...}],'eventData':[{startingTime, endingTime, place}], 'author':*[_id == ^.author._ref]{title}} | order(_createdAt desc) | order(originallyPublished desc)}`) 
        setActiveTag(option)
    }
    ...
    return(
        <div className={`collection-page ${returnColAmount()}`}>
            ...
            <div className="page-navigation-wrapper">
                    <div className="page-navigation-title" onClick={()=>{pageNavigationHandler()}}>
                        CALENDAR
                    </div>
                        {/* <div className="page-navigation-option" 
                        onClick={()=>{setLocationShowing(!locationShowing)}}
                        >
                            <span className="page-navigation-separator">/</span>Location
                        </div> */}
            </div>
            {locationShowing ? 
                <div className="page-navigation-location-wrapper">
                    ...
                    {cities.length > 1 ?
                        cities.map((option, index)=>{
                            return (
                            <div key={'o'+index} className="location-option" onClick={()=>{pageNavigationHandler(option)}}>
                                {index > 0 ? <span className="page-navigation-separator">/</span> : ''}
                    ...
                    <div className="location-search-wrapper">
                        <div className="location-search">
                            <div className="location-search-arrow" onClick={()=>{pageNavigationHandler(searchInput)}}>
                                <HiOutlineArrowSmRight/>
                            </div>
                            <input type="text" placeholder={searchInput} id="search-field" name="search-field" className="page-navigation-search-field-input" onChange={(e)=>{setSearchInput(e.target.value)}} onKeyPress={(e)=>{searchFieldHandler(e)}} />
                        </div>
                    </div>
                </div>
                :
                            ...
                            <div className={`collection-page-article single-width`} >
                                ...
                                <div className="collection-page-article-label">
                                    ...
                                    <div className="collection-page-article-byline">
                                        ...
                                        {article._type == 'event' && article.eventData[0].place !== undefined ? article.eventData[0].place[0].label : ''}<br/>
                                        {'startingTime' in article.eventData[0] ? 
                                            returnFormattedTime(article.eventData[0].startingTime)
                                            :''
                                        }{'endingTime' in article.eventData[0] ? 
                                         ' - ' + returnFormattedTime(article.eventData[0].endingTime)
                                        :''
                                    }
                                    </div>
                                </div>
