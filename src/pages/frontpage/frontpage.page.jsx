...
const Frontpage = () => {
  ...
  useEffect(() => {
    let fetchContentQuery = `{'color':*[_type == 'frontpage']{color}[0],'slideshow':*[_type == 'frontpage']{slideshow[]{..., "image":[slideImage], "video": [video.asset->{...},thumbnail], 'slug':*[_id == ^.slideUrl._ref]{slug, _type}}}, 'articles': *[_type == 'frontpage']{blocks[][0..100]{'block':*[_id == ^._ref]{_id, _type, slug, url, trackingCode,trackingCodeSecond, title,thumbnail, doubleWidth,'highlightItem':highlightItem[0]{'asset':asset->},'eventData':[{startingTime, endingTime,city, place}], 'author':*[_id == ^.author._ref]{title}}[0]}}[0]}`;
    sanityClient
  ...
  const returnCaptionType = (type) => {
    switch (type) {
      case "article":
        return "FEATURES";
        break;
      case "researchreality":
        return "RESEARCH & REALITIES";
        break;
      case "event":
        return "CALENDAR";
      default:
  ...
  const sortArticleRows = () => {
    ...
    if (returnColAmount() == "fiveCol") {
      function addElement() {
        if (clonedPageContent.length > 1) {
          if (c <= 2) {
            // if there are less than two elements in array, any new block can be added
            singleRow.push(clonedPageContent[0].block);
            addToCount(clonedPageContent[0].block);
            clonedPageContent.splice(0, 1);
            addElement();
          } else if (c === 3) {
            // if there are three elements only a double or single can be added
            for (var i = 0; clonedPageContent.length > i; i++) {
              if ("doubleWidth" in clonedPageContent[i].block) {
                if (
                  clonedPageContent[i].block.doubleWidth == "Double column" ||
                  clonedPageContent[i].block.doubleWidth == "Single column"
                ) {
                ...
                // if there's no doubleWidth in the object it means that it's a single width item
                singleRow.push(clonedPageContent[i].block);
                addToCount(clonedPageContent[i].block);
                clonedPageContent.splice(i, 1);
                addElement();
            ...
            // if there are four elements only a single can be added
            for (var i = 0; clonedPageContent.length > i; i++) {
              if ("doubleWidth" in clonedPageContent[i].block) {
      ...
      function addElement() {
        if (clonedPageContent.length > 1) {
          if (c < 2) {
            ...
            // if there are two elements only a double or single can be added
            for (var i = 0; clonedPageContent.length > i; i++) {
              if ("doubleWidth" in clonedPageContent[i].block) {
                if (
                  clonedPageContent[i].block.doubleWidth == "Double column" ||
            ...
            // if there are three elements only a single can be added
            for (var i = 0; clonedPageContent.length > i; i++) {
              if ("doubleWidth" in clonedPageContent[i].block) {
                if (clonedPageContent[i].block.doubleWidth == "Single column") {
                  singleRow.push(clonedPageContent[i].block);
    ...
    if (returnColAmount() == "threeCol") {
      function addElement() {
        if (clonedPageContent.length > 1) {
          if (c === 0) {
            // if there are no elements in the array, any new block can be added
            singleRow.push(clonedPageContent[0].block);
            addToCount(clonedPageContent[0].block);
            clonedPageContent.splice(0, 1);
            addElement();
          } else if (c === 1) {
            // if there is one elements only a double or single can be added
            for (var i = 0; clonedPageContent.length > i; i++) {
              if ("doubleWidth" in clonedPageContent[i].block) {
                if (
                  clonedPageContent[i].block.doubleWidth == "Double column" ||
            ...
            // if there are two elements only a single can be added
            for (var i = 0; clonedPageContent.length > i; i++) {
              if ("doubleWidth" in clonedPageContent[i].block) {
                if (clonedPageContent[i].block.doubleWidth == "Single column") {
                  singleRow.push(clonedPageContent[i].block);
  ...
  return (
    <div className={`frontpage ${returnColAmount()}`}>
      ...
      <div
        className={`frontpage-content-wrapper`}
      ...
      >
        <div className="frontpage-slideshow-section">
          <Swiper
            ...
            onSlideResetTransitionStart={(el) => {
              //next
              if ("slides" in el) {
                  ...
                  {({ isActive }) => (
                    <>
                      {slide.image[0] !== null ? (
                        ...
                        <div className="swiper-video">
                          {isSafari ? (
                                  ...
                                  {!isNarrow ? (
                                    <div className={`safari-video-wrapper`}>
                                      <Player
                                        ...
                                        currentPoster={
                                          "thumbnail" in slide
                                            ? urlFor(slide.thumbnail.asset)
                                                .width(window.innerWidth)
                                                .url()
                                            : "thumbTime" in slide.video[0]
                                            ? `https://image.mux.com/${slide.video[0].playbackId}/thumbnail.jpg?time=${slide.video[0].thumbTime}&width=${window.innerWidth}`
                                            : `https://image.mux.com/${slide.video[0].playbackId}/thumbnail.jpg?time=0&width=${window.innerWidth}`
                                        }
                                        controls="false"
                                        playsinline
                                        viewType="video"
                                        ...
                                        loop
                                        // paused={false}
                                        paused={returnIsActive(isActive)}
                                      ...
                                      >
                                        ...
                                        >
                                          <source
                                            data-src={`https://stream.mux.com/${slide.video[0].playbackId}.m3u8`}
                            ...
                            <SanityMuxPlayer
                              onClick={() =>
                                navigate(slide.slug[0].slug.current)
                              }
                              assetDocument={slide.video[0]}
                              autoload={true}
                              className={"video-inner-wrapper"}
                      ...
                      <div className="swiper-caption-wrapper">
                        <div
                          className="swiper-caption theme-color"
                          onClick={() => navigate(slide.slug[0].slug.current)}
                        >
                          <div className="swiper-caption-type">
                            {returnCaptionType(slide.slug[0]._type)}
                          </div>
                          <div className="swiper-caption-title">
                            {returnFormattedTitle(slide.slideTitle)}
                          </div>
                        </div>
                      </div>
                    </>
              ...
              <div key={"r" + index} className="frontpage-article-row">
                <FadeInSection>
                  {row.map((article) => {
                    ...
                    let isDoubleWidth =
                      "doubleWidth" in article &&
                      article.doubleWidth == "Double column";
                    let isTripleWidth =
                      "doubleWidth" in article &&
                      article.doubleWidth == "Triple column";
                    ...
                    return (
                      ...
                      >
                        <a
                          target={article._type == "ad" ? "_blank" : "_self"}
                          href={
                            "slug" in article
                        ...
                        >
                          <div
                            className={`frontpage-article-image ${
                              article._type !== "ad" ? "theme-color-image" : ""
                            } ${
                              (isDoubleWidth && !oneCol) ||
                              (isTripleWidth && !oneCol)
                                ? "height-ratio-100"
                                : imageHeight.ratio
                            }`}
                          >
                            {"highlightItem" in article ? (
                              ...
                              "sanity.imageAsset" ? (
                                <>
                                  {article.highlightItem.asset.extension ==
                                  "gif" ? (
                                    <img
                                      src={urlFor(
                                        article.highlightItem.asset._id
                          ...
                          <div className="frontpage-article-label">
                            ...
                            <div className="frontpage-article-byline">
                              {article.author.length > 0
                                ...
                                : ""}
                              {article._type == "event" &&
                              article.eventData[0].city !== undefined
                                ? article.eventData[0].city[0].label
                                : ""}
                              <br />
                              {article._type == "event" &&
                            ...
                            {"trackingCode" in article
                              ? returnInsetIframeHTML(article.trackingCode)
                              : ""}
                          </div>
                        </a>
                      </div>
