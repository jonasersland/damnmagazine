...
import AccordionNewsletter from "../accordion/accordionNewsletter.component"
...
const Menu = () =>{
    ...
    const {menuState, setMenuStateHandler} = useContext(GlobalContext);
    ...
    useEffect(() =>{ 
        ...
        .then(data => {setIssue(data.latestIssueData.latestIssueNumber);setIssueUrl(data.latestIssueData.latestIssueUrl)})
        .catch(console.error)
    },[]);
...
<path className="damn-logo" d="M153.41,5.27c-0.05,0.24-0.08,0.48-0.14,0.71c-0.48,1.8-1.64,2.91-3.45,3.31c-2.3,0.5-4.55-1-5.13-3.21
...
<path className="damn-logo" d="M66.53,34.98V0.01h0.25c3.12,0,6.25,0,9.37-0.01c0.2,0,0.27,0.07,0.33,0.25c1.85,5.97,3.71,11.94,5.57,17.91
...
<path className="damn-logo" d="M116.41,34.98h-6.24v-0.27c0-11.48,0-22.95-0.01-34.43c0-0.22,0.06-0.28,0.28-0.28c2.68,0.01,5.37,0.01,8.05,0
	...
	V34.98z"/>
<path className="damn-logo" d="M0,34.98V0.01h0.25c3.08,0,6.15-0.02,9.23,0.01c1.17,0.01,2.35,0.11,3.52,0.21c1.95,0.16,3.85,0.54,5.71,1.16
...
<path className="damn-logo" d="M38.83,27.64c-0.38,1.06-0.76,2.11-1.13,3.16c-0.48,1.35-0.96,2.69-1.44,4.04c-0.04,0.1-0.07,0.17-0.2,0.17
    ...
    return(
        <div className="menu-wrapper">
            <div className="menu-static">
                <div className="menu-section-logo theme-color-on-hover" onClick={()=>navigate('/')}>
                    <DAMNlogo/>
                </div>
                
                <div className={`menu-slogan ${!searchFieldActive ? 'slogan-visible' : ''}`}>
                ...
                {searchFieldActive &&
                    <div className="menu-search-wrapper">
                        <div className="menu-search-field">
                            <input type="text" placeholder='Search' id="search-field" name="search-field" className="search-field-input" onChange={(e)=>{setSearchInput(e.target.value)}} onKeyPress={(e)=>{searchFieldHandler(e)}} />
                            {/* <input type="text" placeholder='Search' id="search-field" name="search-field" className="search-field-input" onChange={(e)=>{setSearchInput(e.target.value)}} onSubmit={()=>{submitSearchHandler()}}/> */}
                            <div className="menu-search-button theme-color" onClick={()=>{submitSearchHandler()}}>
                                <BiSearch/>
                            </div>
                        </div>
                    </div>
                ...
                <div className="menu-section-right">
                    <div className="menu-section-right-buy menu-section-title">
                        ...
                        <a target="_blank" className="a-no-style new theme-color" href={issueUrl}>
                                BUY ISSUE <span className="menu-issue-number">{issue}</span>
                        </a>
                    </div>
                    <div className="menu-section-right-buttons">
                        <div className="menu-section-right-search-button theme-color" onClick={()=>{searchFieldToggleHandler(!searchFieldActive)}}>
                        {searchFieldActive ? <GrClose/> : <BiSearch/>}
                        </div>
                        <div className="menu-section-right-menu-button theme-color" onClick={()=>{accordionToggleHandler(!menuState);toggleNewsletterExpanded(false)}}>
                            {menuState ? <GrClose/> : <AiOutlineMenu/>}
                        </div>
                    </div>
                </div>
            </div>
                ...
                {/* <Accordion isOpen={isOpen}> */}
                    <div className={`menu-section-expanded ${menuState ? 'menu-section-expanded-display' : ''}`}>
                        ...
                        <div className="menu-section-newsletter-wrapper">
                            <div className="menu-section-newsletter">
                                <div className="menu-section-title theme-color" onClick={()=>toggleNewsletterExpanded(!newsletterExpanded)}>
                                    NEWSLETTER
                                </div>
                            </div>
                            <div className="menu-section-social-buttons">
                                <a target="_blank" className="a-no-style theme-color" href='https://www.instagram.com/damn_magazine/'>
                                    <span><FaInstagramSquare/></span>
                                </a>
                                <a target="_blank" className="a-no-style theme-color" href='https://www.facebook.com/damnmagazine.net'>
                                    <span><FaFacebookSquare/></span>
                                </a>
                                <a target="_blank" className="a-no-style theme-color" href='https://twitter.com/DAMN_magazine'>
                ...
                <div className={`menu-section-newsletter-expanded ${newsletterExpanded ? 'menu-section-newsletter-expanded-display' : ''}`}>
                    <div className="menu-section-newsletter-content">
                        <NewsletterSubscribe/>
                        ...
                        <div className="menu-section-email-submit menu-section-item theme-color" onClick={()=>submitNewsletter()}>
